// G.711 A-Law / u-Law conversion tables and functions
...
/**
 * Convert buffer from A-Law to u-Law
 * Use this when receiving from Telnyx (A-Law) and sending to Frontend (u-Law)
 */
function alawToMulaw(buffer) {
  const output = Buffer.alloc(buffer.length);
  for (let i = 0; i < buffer.length; i++) {
    const pcm = alawMap[buffer[i]];
    output[i] = linear2ulaw(pcm);
  }
  return output;
}

/**
 * Convert buffer from u-Law to A-Law
 * Use this when receiving from Frontend (u-Law) and sending to Telnyx (A-Law)
 */
function mulawToAlaw(buffer) {
  const output = Buffer.alloc(buffer.length);
  for (let i = 0; i < buffer.length; i++) {
    const pcm = ulawMap[buffer[i]];
    output[i] = linear2alaw(pcm);
  }
  return output;
}
